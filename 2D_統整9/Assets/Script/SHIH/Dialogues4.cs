...
public class Dialogues4 : MonoBehaviour
{
    //这是场景中的各个物体
    ...
    public void Start()
    {
        ...
        //载入资源文件
        string data = Resources.Load("dialogues4").ToString();//注意这里没有后缀名xml。你可以看看编辑器中，也是不带后缀的。因此不要有个同名的其它格式文件注意！
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))//如果点击了鼠标左键
        {
            ...
            { //对话完了
                ...
                UIManager2.Instance.ClosePanel("SHIH/Panel4");
    ...
    private void dialogues_handle(int dialogue_index)
    {
        ...
        {   //显示当前说话的角色
            case "A":
                ...
                //roleC.SetActive(false);
                GameObject.Find("玉米A").GetComponent<SpriteRenderer>().color = new Color(0, 0, 0, 0);
                ...
                roleName.GetComponent<Text>().text = "飛機頭:";
                break;
            case "B":
                //roleB.SetActive(false);
                //roleA.SetActive(false);
                ...
                GameObject.Find("玉米A").GetComponent<SpriteRenderer>().color = new Color(0, 0, 0, 0);
                roleName.GetComponent<Text>().text = "馬鈴薯:";
                break;
        }
