...
public class BossController : MonoBehaviour
{
    ...
    private bool MonsterAttack(Vector3Int Pos)//玉米的範圍攻擊
    {
        for (int x = -1; x < 2; x++)
        {
            for (int y = -1; y < 2; y++)
    ...
    {
        if (Mathf.Abs(road[road.Count - 1].x - road[walk].x) > 1 || Mathf.Abs(road[road.Count - 1].y - road[walk].y) > 1)
        {
        ...
        else if (Mathf.Abs(road[road.Count - 1].x - road[walk].x) <= 1 && Mathf.Abs(road[road.Count - 1].y - road[walk].y) <= 1)
        {
            walk = 0;
            go = false;
            attack = true;
        ...
        /*
        else if (walk == 2 && road.Count == 3)
        {
            walk = 0;
            go = false;
            Movelock = true;
        }
        else if (walk == 1 && road.Count == 2)
        {
            walk = 0;
            go = false;
            Movelock = true;
        }
        else if (walk == 0 && road.Count == 1)
        {
            walk = 0;
            go = false;
            Movelock = true;
